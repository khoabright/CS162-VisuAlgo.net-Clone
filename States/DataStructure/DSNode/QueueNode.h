...
struct QueueNode
{
    ...
    std::vector<QueueNode*> array_next;
    ...
    QueueNode();
    QueueNode(float x, float y, float scale_x, float scale_y, int _val, sf::Font *_font, std::map<std::string, sf::Color>* Colors);

    void reset();

    //Next
    void updateNext();
    void newNext(QueueNode* new_next);
    void prevNext();
    void nextNext();
