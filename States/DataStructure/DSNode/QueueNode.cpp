...
QueueNode::QueueNode(float x, float y, float scale_x, float scale_y, int _val, sf::Font *_font, std::map<std::string, sf::Color> *Colors)
{
    this->Colors = Colors;
...
//Next
void QueueNode::updateNext()
{
    this->next = array_next[idx_next];
}

void QueueNode::newNext(QueueNode *new_next)
{
...
void QueueNode::prevNext()
{
    this->idx_next--;
    this->updateNext();
}

void QueueNode::nextNext()
{
    this->idx_next++;
    this->updateNext();
}
...
//Val
void QueueNode::updateVal()
{
    this->val = array_val[idx_val];
}

void QueueNode::newVal(int new_val)
{
...
void QueueNode::prevVal()
{   
    this->idx_val--;
    this->updateVal();
}

void QueueNode::nextVal()
{
    this->idx_val++;
    this->updateVal();
}
...
void QueueNode::newLabel(std::string new_label)
{
...
void QueueNode::prevLabel()
{
    this->idx_label--;
    this->updateLabel();
}

void QueueNode::nextLabel()
{
    this->idx_label++;
    this->updateLabel();
}
...
void QueueNode::newPos(sf::Vector2f new_pos)
{
...
void QueueNode::prevPos()
{
    this->idx_pos--;
    this->updatePos();
}

void QueueNode::nextPos()
{
    this->idx_pos++;
    this->updatePos();
}
...
//Render
void QueueNode::renderAnimation()
{
}

void QueueNode::render(sf::RenderTarget *target)
{
    this->updateText();
    this->updateLabel();
