...
struct SLLNode
{
    ...
    SLLNode();
    SLLNode(float x, float y, float scale_x, float scale_y, int _val, sf::Font *_font, std::map<std::string, sf::Color>* Colors);

    void reset();

    //Next
    void updateNext();
    void newNext(SLLNode* new_next);
    void prevNext();
    void nextNext();
