...
AnimationSLL::AnimationSLL(std::map<std::string, sf::Color> *Colors, float animationTime, CodeHighlight *codeHighlight)
...
void AnimationSLL::addCodeLines(std::vector<int> codeLines)
{
...
void AnimationSLL::changeHead(SLLNode *&head, SLLNode *old_head, SLLNode *targetNode, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex])
        return;
    this->startStep[curChildIndex] = 0;
...
void AnimationSLL::changeTail(SLLNode *&tail, SLLNode *old_tail, SLLNode *targetNode, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex])
        return;
    this->startStep[curChildIndex] = 0;
...
void AnimationSLL::showNode(SLLNode *targetNode, std::string targetLabel, int &numberNode, std::vector<int> codeLines) 
{   
    /* For newNode only */
...
void AnimationSLL::hideNode(SLLNode *targetNode, int &numberNode, std::vector<int> codeLines)
{   
...
void AnimationSLL::changeNodeLabel(SLLNode *targetNode, std::string targetLabel, std::vector<int> codeLines)
{   
...
void AnimationSLL::changeNodeValue(SLLNode *targetNode, int newValue, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex]) return;
    this->startStep[curChildIndex] = 0;
...
void AnimationSLL::connectNodes(SLLNode *targetNode, SLLNode *nextNode, std::vector<int> codeLines)
{   
    if (this->reverse && this->doneStep)
    {   
        targetNode->prevNext();
...
void AnimationSLL::highlightCurrentNode(SLLNode *targetNode, std::string targetLabel, std::string targetColor, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex]) return;
    this->startStep[curChildIndex] = 0;
...
void AnimationSLL::highlightPassedNode(SLLNode *targetNode, std::string passedLabel, std::string currentLabel, std::string passedColor, std::string currentColor, std::vector<int> codeLines)
{      
    if (!this->startStep[curChildIndex]) return;
    this->startStep[curChildIndex] = 0;
...
void AnimationSLL::moveNode(SLLNode *targetNode, float x1, float y1)
{
    if (this->reverse && this->doneStep)
    {   
        targetNode->prevPos();
...
void AnimationSLL::makeArrow(sf::CircleShape *node1, sf::CircleShape *node2, sf::RectangleShape *targetArrow)
{
...
void AnimationSLL::Relayout(bool emptyList, SLLNode *head, SLLNode *tail, float start_x, float start_y, float distance, std::vector<int> codeLines)
{       
    if (emptyList) {
        if (!this->startStep[curChildIndex]) return;
        this->startStep[curChildIndex] = 0;
    ...
    if (this->startStep[curChildIndex]) {
        this->startStep[curChildIndex] = 0;
        for (SLLNode *cur = head; cur != tail->next; cur = cur->next) {
    ...
    /* Show Arrow */
    for (SLLNode *cur = head; cur != tail; cur = cur->next)
    {
        if (cur != tail)
        {
...
//Reset
void AnimationSLL::updateNodePosition(SLLNode *head, SLLNode *tail, float start_x, float start_y, float distance)
{   
    if (head == nullptr)
        return;
...
void AnimationSLL::newInstruction(SLLNode *head, SLLNode *tail, float start_x, float start_y, float distance)
{   
