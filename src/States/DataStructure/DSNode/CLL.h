...
class CLL : public DataStructure
{
private:
    // Variables
    ...
    float nodeDistanceY = 30.f;

    CLLNode *head, *tail;
    AnimationCLL *animationCLL;
    std::vector<CLLNode *> Nodes;
    ...
    void addMiddle(int index, int nodeValue, bool addHead, bool addTail);

    // Delete
    void deleteHead();
    void deleteTail();
    void deleteMiddle(int index, bool delHead, bool delTail);
...
public:
    CLL(sf::RenderWindow *window, std::map<std::string, int> *supportedKeys, std::stack<State *> *states);
    ~CLL();
