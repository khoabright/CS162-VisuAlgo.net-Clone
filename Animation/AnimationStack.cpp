...
AnimationStack::AnimationStack(std::map<std::string, sf::Color> *Colors, float animationTime, CodeHighlight *codeHighlight)
...
void AnimationStack::addCodeLines(std::vector<int> codeLines)
{
...
void AnimationStack::changeHead(StackNode *&head, StackNode *old_head, StackNode *targetNode, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex])
        return;
    this->startStep[curChildIndex] = 0;
...
void AnimationStack::changeTail(StackNode *&tail, StackNode *old_tail, StackNode *targetNode, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex])
        return;
    this->startStep[curChildIndex] = 0;
...
void AnimationStack::showNode(StackNode *targetNode, std::string targetLabel, int &numberNode, std::vector<int> codeLines) 
{   
    /* For newNode only */
...
void AnimationStack::hideNode(StackNode *targetNode, int &numberNode, std::vector<int> codeLines)
{   
...
void AnimationStack::changeNodeLabel(StackNode *targetNode, std::string targetLabel, std::vector<int> codeLines)
{   
...
void AnimationStack::changeNodeValue(StackNode *targetNode, int newValue, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex]) return;
    this->startStep[curChildIndex] = 0;
...
void AnimationStack::connectNodes(StackNode *targetNode, StackNode *nextNode, std::vector<int> codeLines)
{   
    if (this->reverse && this->doneStep)
    {   
        targetNode->prevNext();
...
void AnimationStack::highlightCurrentNode(StackNode *targetNode, std::string targetLabel, std::string targetColor, std::vector<int> codeLines)
{   
    if (!this->startStep[curChildIndex]) return;
    this->startStep[curChildIndex] = 0;
...
void AnimationStack::highlightPassedNode(StackNode *targetNode, std::string passedLabel, std::string currentLabel, std::string passedColor, std::string currentColor, std::vector<int> codeLines)
{      
    if (!this->startStep[curChildIndex]) return;
    this->startStep[curChildIndex] = 0;
...
void AnimationStack::moveNode(StackNode *targetNode, float x1, float y1)
{
    if (this->reverse && this->doneStep)
    {   
        targetNode->prevPos();
...
void AnimationStack::makeArrow(sf::CircleShape *node1, sf::CircleShape *node2, sf::RectangleShape *targetArrow)
{
...
void AnimationStack::Relayout(bool emptyList, StackNode *head, StackNode *tail, float start_x, float start_y, float distance, std::vector<int> codeLines)
{       
    if (emptyList) {
        if (!this->startStep[curChildIndex]) return;
        this->startStep[curChildIndex] = 0;
    ...
    if (this->startStep[curChildIndex]) {
        this->startStep[curChildIndex] = 0;
        for (StackNode *cur = head; cur != tail->next; cur = cur->next) {
    ...
    /* Show Arrow */
    for (StackNode *cur = head; cur != tail; cur = cur->next)
    {
        if (cur != tail)
        {
...
//Reset
void AnimationStack::updateNodePosition(StackNode *head, StackNode *tail, float start_x, float start_y, float distance)
{   
    if (head == nullptr)
        return;
...
void AnimationStack::newInstruction(StackNode *head, StackNode *tail, float start_x, float start_y, float distance)
{   
